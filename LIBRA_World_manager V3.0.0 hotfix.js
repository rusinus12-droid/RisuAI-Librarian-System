...
@media (max-width:640px){
  #${OVERLAY_ID}{left:12px;right:12px;top:12px}
            ...
            if (sourceSummaries.length <= HIERARCHICAL_SYNTHESIS_MAX_BATCHES && directInput.length <= SYNTHESIS_MAX_INPUT_CHARS) {
                for (let attempt = 0; attempt < 2; attempt++) {
                    try {
                        ...
                        const parsed = extractJson(synthesisResult?.content || '');
            ...
            for (let i = 0; i < currentBatches.length; i++) {
                ...
                for (let attempt = 0; attempt < 2 && !parsed; attempt++) {
                    try {
                        ...
                        parsed = extractJson(synthesisResult?.content || '');
                    } catch (e) {
                ...
                for (let i = 0; i < reportBatches.length; i++) {
                    ...
                    for (let attempt = 0; attempt < 2 && !parsed; attempt++) {
                        try {
                            ...
                            parsed = extractJson(mergeResult?.content || '');
                        } catch (e) {
                ...
                return runMaintenanceLLM(() =>
                    (async () => {
                        ...
                        if (LLMProvider.isConfigured(MemoryEngine.CONFIG, 'aux')) {
                            ...
                            auxDraft = extractJson(auxResult?.content || '');
                        }
                    ...
                    try {
                        ...
                        for (let i = 0; i < reviewWindows.length; i++) {
                            ...
                            if (hasAuxReviewer) {
                                ...
                                reviewed = extractJson(reviewResult?.content || '');
                            }
                            ...
                            if (hasPrimaryArbiter) {
                                ...
                                const arbitedWindow = extractJson(cycleArbiterResult?.content || '');
                                if (arbitedWindow) {
            ...
            if (updates.personality) {
                for (const key of ['traits', 'values', 'fears', 'likes', 'dislikes', 'sexualPreferences']) {
                    if (Array.isArray(updates.personality[key])) {
                        const normalizedItems = [...new Set(updates.personality[key].filter(Boolean))];
                        if (forceReplace) {
                ...
                if (updates.personality.sexualOrientation && (forceReplace || !entity.personality.sexualOrientation)) {
...
<div id="toast" class="toast"></div>
    ...
    const show = async () => {
        ...
        const reanalyzeWorldFromChat = async () => {
            const activeChat = char?.chats?.[char.chatPage];
            if (!char || !activeChat) throw new Error("채팅방을 찾을 수 없습니다.");
            const msgs = buildAnalyzableMessages(activeChat);
            if (msgs.length === 0) throw new Error("재분석할 대화 내역이 없습니다.");
        ...
        const renderWorld = () => {
            ...
            if (effectiveRules) {
                ...
                if (Array.isArray(physics.special_phenomena) && physics.special_phenomena.length > 0) {
                    lines.push(physics.special_phenomena.map(v => String(v || '').trim()).filter(Boolean).join(', '));
                }
        ...
        const buildSettingsConfigFromUI = () => {
            ...
            return {
                ...
                coldStartHistoryLimit: resolveColdStartHistoryLimit(coldStartScopePreset, 0),
